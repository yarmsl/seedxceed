...
const Brands = ({brands = [], isFetching}: IBrandsProps): JSX.Element => {
  ...
  const brandDataChart = useMemo(() => {
    return brands.map((brand: IBrandsState) => {
      return {...brand, graph: Object.keys(brand.graph).map(date => ({...brand.graph[date], date: formatDate(date)}))}
    })
  },[brands])

  const oneBrandData = useCallback((idx: number) => {
    return brandDataChart[idx]
  }, [brandDataChart])

  const graphData = useMemo(() => {
    return oneBrandData(swiperIndex) != null ? oneBrandData(swiperIndex).graph : []
  }, [oneBrandData, swiperIndex])

  const topSalesBrand = useMemo(() => {
    return oneBrandData(swiperIndex) != null ? oneBrandData(swiperIndex).top_5_products : []
  }, [oneBrandData, swiperIndex])

  const summaryBrand = useMemo(() => {
    return oneBrandData(swiperIndex) != null ? oneBrandData(swiperIndex).summary : {sales: 0, orders: 0, days_on_sale: 0, returns: 0, products_count: 0}
  }, [oneBrandData, swiperIndex])
